...
import { supabase } from "@/lib/supabaseClient" // 올바른 경로로 수정
import { useAuth } from "@/components/AuthProvider" // 올바른 경로로 수정
import { motion } from "framer-motion"
import { Input } from "@/components/ui/input" // 올바른 경로로 수정
import { Button } from "@/components/ui/button" // 올바른 경로로 수정
import { Label } from "@/components/ui/label" // 올바른 경로로 수정
import Header from "@/components/header" // 올바른 경로로 수정
...
const handleOAuthLogin = async (provider: Provider) => {
  console.log('=== handleOAuthLogin 시작 ==='); // 디버깅 로그 추가
  
  setLoading(true);
  setMessage(null);
  
  // 현재 페이지 URL에서 redirect 파라미터 가져오기
  const redirect = searchParams.get('redirect') || '/blog';
  console.log('Redirect param:', redirect); // 디버깅 로그
  
  // 중요: redirectTo URL에 next 파라미터를 직접 포함
  const redirectToUrl = `${window.location.origin}/auth/callback?next=${encodeURIComponent(redirect)}`;
  console.log('Final redirectTo URL:', redirectToUrl); // 디버깅 로그
  ...
  console.log('OAuth options:', options); // 디버깅 로그
  console.log('Supabase client auth config:', JSON.stringify(supabase.auth, null, 2)); // Supabase 설정 확인
  ...
  //   });
  //   // PKCE 플로우에서는 redirectTo를 직접 설정할 필요가 없습니다.
  //   // Supabase가 자동으로 /auth/callback으로 리디렉션합니다.
  //   // 이 URL은 Supabase 프로젝트의 Redirect URLs에 등록되어 있어야 합니다.
  //   const callbackUrl = `${window.location.origin}/auth/callback?${params.toString()}`;

  //   console.log(`[LoginPage] handleOAuthLogin - Redirect param from URL: ${searchParams.get('redirect')}`);
  //   console.log(`[LoginPage] handleOAuthLogin - Final redirect path for callback (next): ${redirect}`);
  //   console.log(`[LoginPage] handleOAuthLogin - Callback URL for Supabase: ${callbackUrl}`);
  //   console.log(`[LoginPage] handleOAuthLogin - Provider: ${provider}`);

  //   const oauthOptions: any = {
  //     // redirectTo: callbackUrl, // 이 줄을 제거합니다.
  //   };
